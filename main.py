        ...
        if message.guild is None:
            ...
            if "hello" in user_text or "hi" in user_text:
                await message.author.send(
                    f"Greetings, Commander {message.author.display_name}! ⚔️"
                    " How can I assist the alliance today?"
                )
            elif "status" in user_text:
                await message.author.send(
                    "All systems operational. I currently have"
                    f" {len(self.active_schedules)} upcoming events scheduled."
                )
            else:
                await message.author.send(
                    "I am monitoring the realm. Use `/schedule` in the server"
                    " to set an alliance reminder, or ask me for `status`!"
                )
            ...
            if event["time"] == current_time_str:
                ...
                if channel:
                    embed = discord.Embed(
                        ...
                        description=(
                            f"The scheduled event **{event['name']}** is"
                            " starting now! Prepare your marches."
                        ),
...
@client.tree.command(
    name="schedule",
    description="Schedule an automatic alliance announcement ping.",
)
@app_commands.describe(
    event_name="Name of event (e.g. Bear Hunt)",
    time_utc="Time in 24h UTC format (e.g. 10:00)",
)
async def schedule(
    interaction: discord.Interaction, event_name: str, time_utc: str
):
    # BULLETPROOF PERMISSION CHECK:
    if not interaction.permissions or not interaction.permissions.administrator:
        await interaction.response.send_message(
            "❌ Only Alliance Leaders with Administrator permissions can"
            " schedule alerts.",
            ephemeral=True,
        )
        return
    ...
    try:
        ...
        await interaction.response.send_message(
            "❌ Invalid time format! Please use HH:MM 24-hour format (e.g.,"
            " `09:30` or `14:00`).",
            ephemeral=True,
        )
        return
    ...
    await interaction.response.send_message(
        "✅ **Success!** I will automatically ping `@everyone` for"
        f" **{event_name}** at exactly **{time_utc} UTC** in the announcements"
        " channel."
    )
...
@client.tree.command(
    name="rally",
    description="Issue an urgent coordinate alert to the entire alliance.",
)
@app_commands.describe(
    target="Who are we hitting?", coordinates="Coordinates (e.g., X:150 Y:340)"
)
async def rally(interaction: discord.Interaction, target: str, coordinates: str):
    # BULLETPROOF PERMISSION CHECK:
    if not interaction.permissions or not interaction.permissions.administrator:
        await interaction.response.send_message(
            "❌ Only Alliance Leaders can use this command.", ephemeral=True
        )
        return
    ...
    embed.set_footer(
        text=f"Issued by Commander {interaction.user.display_name}"
    )
